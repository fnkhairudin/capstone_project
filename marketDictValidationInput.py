...
def add():
    ...
    nameFruit = pypi.inputStr(prompt='Masukkan nama buah: ', applyFunc=lambda x: x.capitalize(), blockRegexes=[r'.*'],allowRegexes=[r'[a-zA-Z]'])
    countFruit = pypi.inputInt(prompt='Masukkan jumlah buah: ', min=1) #min untuk lebih dari atau sama dengan
    priceFruit = pypi.inputInt(prompt='Masukkan harga buah: ', greaterThan=0)
    # Loop item di dalam listFruit
...
def delete():
    ...
    index = pypi.inputInt(prompt='Masukkan index buah yang ingin dihapus: ', min=0)
...
def buy():
    ...
    while True:
        ...
        index = pypi.inputInt(prompt='Masukkan index buah yang ingin dibeli: ', min=0)
        nameFruit = listFruit[f'buah-{index}']
        countFruit = pypi.inputInt(prompt='Masukkan jumlah buah yang ingin dibeli: ', min=1)
        ...
        # Selain itu, tambahkan ke 'listChart'
            ...
            reorder = pypi.inputYesNo(prompt='Apakah anda ingin keluar(ya/tidak): ', yesVal='ya', noVal='tidak')
            if reorder.lower() == "tidak": # 'tidak' berarti belanja telah usai, 'ya' berarti ingin belanja lagi
                break
    ...
    while True:
        ...
        pay = pypi.inputInt(prompt='Masukkan jumlah uang: ', min=price)
...
"""
        ...
        menuNumber = pypi.inputInt(prompt='Masukkan angka menu yang ingin dijalankan: ', greaterThan=0, lessThan=6)
